...
export default function Authentication() {
  ...
  const SignInCard = () => {
    ...
    const onEmailCangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
      setError(false);
      const { value } = event.target;
      setEmail(value);
    }
    //  event handler: 비밀번호 변경 이벤트 처리
    const onPasswordCangeHandler = (event: ChangeEvent<HTMLInputElement>) => {
      setError(false);
      const { value } = event.target;
      setPassword(value);
    }
    ...
    const onSignUpLonkClickHandler = () => {
      setView('sign-up');
    }
      ...
      <div className='auth-card'>
        <div className='auth-card-box'>
          <div className='auth-card-top'>
            ...
            <InputBox ref={emailRef} label='이메일 주소' type='text' placeholder='이메일 주소를 입력해주세요.' error={error} value={email} onChange={onEmailCangeHandler} onKeyDown={onEmailKeyDownHandler} />
            <InputBox ref={passwordRef} label='패스워드' type='password' placeholder='비밀번호를 입력해주세요' error={error} value={password} onChange={onPasswordCangeHandler} icon={passwordButtonIcon} onButtonClick={onPasswordButtonClickHandler} onKeyDown={onPasswordKeyDownHandler} />
          </div>
            ...
            <div className='auth-description-box'>
              <div className='auth-description'>{'신규 사용자이신가요? '}<span className='auth-description-link' onClick={onSignUpLonkClickHandler}>{' 무료 회원가입'}</span></div>
            </div>
          </div>
        </div>
      </div>
  ...
  const SignUpCard = () => {
    ...
    const [isAgreedPersonlaError, setAgreedPersonalError] = useState<boolean>(false);
    ...
    //  event handler: 다음 확인 버튼 클릭 이벤트 처리
    ...
    const onSignUpButtonClickHandler = () => {
      ...
      const hasAddress = address.length > 8;
      if (!hasAddress) {
    ...
    //  event handler: 핸드폰 번포 키 다운 이벤트 처리
          ...
          <div className='auth-card-bottom'>
            ...
            {page === 2 && (
              ...
              <div className='auth-consent-box' >
                ...
                <div className={isAgreedPersonlaError ? 'auth-consent-title-error' : 'auth-consent-title'}>{'개인정보동의'}</div>
                <div className='auth-constent-link'>{'더보기 >'}</div>
              </div>
        ...
        <div className='auth-jumbotron-box'>
          <div className='auth-jumbotron-contents'>
            ...
            <div className='auth-jumbotron-text-box'>
              ...
              <div className='auth-jumbotron-text'>{'JINHO BOARD 입니다.'}</div>
            </div>
          </div>
        </div>
